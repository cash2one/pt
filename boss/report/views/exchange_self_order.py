...
@login_required
@add_common_var
def exchange_self_order(request, template_name):
    app = request.GET.get("app")
    cur_order_type = request.GET.get("product")
    if not cur_order_type:
        cur_order_type = None
    elif int(cur_order_type):
        cur_order_type = int(cur_order_type)
    ...
    cursor.execute("call `SP_T_RP_D_TURNOVER_SELF_PRODUCT_SUMMARY`(%s, %s, %s, %s, %s, %s, %s)",
                    [get_datestr(1, "%Y-%m-%d"),today.strftime("%Y-%m-%d"), cur_order_type, None, None, None, 4])
...
def exchange_self_order_summaries(request):
...
def get_exchange_self_order_table_data(request):
    today = datetime.datetime.now()
...
def exchange_self_order_reports_csv(request):
