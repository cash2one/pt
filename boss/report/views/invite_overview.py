...
@login_required
@add_common_var
def invite_overview(request, template_name):
    ...
    cursor.execute("call `SP_T_RP_D_INVITE_AND_REWARD_SUMMARY`(%s, %s, %s)",
...
def get_invite_overview_table_data(request):
    today = datetime.datetime.now()
...
def invite_overview_reports_csv(request):
    start_date = request.GET.get("start_date")
    end_date = request.GET.get("end_date")
    filename = '%s(%s-%s).csv' % ("邀新有礼汇总分析", str(start_date), str(end_date))
    csv_data = [["日期",
                "邀请人总数",
                "被邀请人总数",
                "被邀请人-首单下单成功总数",
