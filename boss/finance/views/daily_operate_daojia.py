...
def get_data(request, start_date, end_date, app):
    ...
    cursor.execute("call `SP_T_RP_DUIZHANG_DAOJIA_OPERATION_SUMMARY_APP`(%s, %s, %s)",
                    [start_date, end_date, app])
...
@login_required
@permission_required(u'man.%s' % FinanceConst.FINANCE_OPERATION_DAOJIA_OPERATION_SUMMARY, raise_exception=True)
@add_common_var
def daily_operate_daojia(request, template_name):
    return report_render(request, template_name, {"currentdate": get_datestr(1, "%Y-%m-%d")})

@login_required
@permission_required(u'man.%s' % FinanceConst.FINANCE_OPERATION_DAOJIA_OPERATION_SUMMARY, raise_exception=True)
def daily_operate_daojia_ajax(request):
...
@login_required
@permission_required(u'man.%s' % FinanceConst.FINANCE_OPERATION_DAOJIA_OPERATION_SUMMARY, raise_exception=True)
def daily_operate_daojia_csv(request):
