...
def get_sum_data(start_date, end_date, app):
    ...
    cursor.execute("call `SP_T_RP_D_OPERATION_DATA_SUMMARY_OVER_MONTH_APP`(%s, %s, %s, %s, %s, %s)",
                    [start_date, end_date, app, None, None,1])
    data = []
...
@login_required
@permission_required(u'man.%s' % FinanceConst.FINANCE_DETAIL_OVERTIME_SELF_ORDER_DETAIL, raise_exception=True)
@add_common_var
def over_month_order(request, template_name):
...
@login_required
@permission_required(u'man.%s' % FinanceConst.FINANCE_DETAIL_OVERTIME_SELF_ORDER_DETAIL, raise_exception=True)
def over_month_order_ajax(request):
...
@login_required
@permission_required(u'man.%s' % FinanceConst.FINANCE_DETAIL_OVERTIME_SELF_ORDER_DETAIL, raise_exception=True)
def over_month_order_csv(request):
